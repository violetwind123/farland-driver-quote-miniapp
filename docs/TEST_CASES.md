...
6. 接送需求卡显示：
   ...
   - 行李
   - 由 Farland 顾问代您提交
7. 运营状态文案显示：`Farland 已为您筛选以下优选用车方案。`
8. 首页只显示需求摘要和 `查看用车方案` 按钮，不把 3 个报价全部堆在日程页。
9. 点击 `查看用车方案` 进入 `pages/customer/transfer-detail/transfer-detail?request_id=...`。
10. Transfer Detail 页面显示：
    - 需求快照
    - 运营状态
    - 优选用车方案
    - 处理进度
11. Transfer Detail 页面显示 3 个优选报价方案。
12. 每个报价卡显示：
   - 方案标题
   - 车型
   - 容量
...
13. 每个报价卡必须拆开显示：
    ...
    - 预计总价
14. 推荐报价显示 `推荐` badge。
15. 点击 `选择此方案` 后，显示 `已选择方案，等待 Farland 最终确认`。
16. 活动时间线显示：
    - 接送需求已提交
    - Farland 开始确认优选用车方案
    - 已发布 3 个优选用车方案
17. 页面显示 `访校包车服务` section。
18. 包车卡显示 Trip 层信息和 Day/Segment 层信息。
19. 页面显示 `已确认接送` section。
