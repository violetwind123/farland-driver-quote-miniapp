...
> **写代码铁律:实现功能不得突破主线。** 主线是第一优先,功能是第二。若某个功能**看起来需要突破主线**才能做到(例:为"运营预览可返回"而牺牲"客户界面带 bottombar"、为"省事"让 sheet 展示壳自渲染 days、为"确认 gate"去关 opsUpsert 自动发布),那是**停下来的信号**,不是绕过的理由——先回主线/找 owner 确认要不要改主线,改定了再写;**绝不用突破主线的实现去凑功能**。突破主线的代码即使跑通也是错的。
> 反面教训(已发生并纠正):op_preview 曾用"非 tab home 内联渲染"实现"运营预览可返回",牺牲了主线不变式 #4 的 bottombar → 预览不忠实。正确解是**落真 tab(带 bottombar)+ 加退出条**,两者都满足主线。
