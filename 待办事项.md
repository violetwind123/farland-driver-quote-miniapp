...
- Feature completed: replaced the customer home next-trip card with a customer registration entry.
- Files changed:
  - `cloudfunctions/registerCustomerProfile/index.js`
  - `cloudfunctions/registerCustomerProfile/package.json`
...
- Notes:
  ...
  - Registration binds the current WeChat identity through `cloud.getWXContext().OPENID`; the frontend does not pass `openid`.
  - Empty home no longer shows the explanatory line under `暂无行程安排`; it keeps only `联系顾问`.
- Next recommended task:
  - Deploy `registerCustomerProfile` and `getCustomerHome`, then preview the empty customer home state on a new WeChat account.
