...
async function buildRegionSummary() {
  const driverRes = await db.collection('drivers').where({ status: 'active' }).limit(1000).get();
  const regionMap = {};
  ...
  for (const driver of driverRes.data) {
    ...
    if (driver.default_vehicle_id) {
      ...
      const vehicle = vehicleRes && vehicleRes.data;
      if (vehicle) {
...
exports.main = async () => {
  try {
    ...
    const requestRes = await db.collection('ride_requests').orderBy('created_at', 'desc').limit(100).get();
    ...
    const requests = await Promise.all(requestRes.data.map(async (request) => {
      ...
      if (ACTIVE_REQUEST_STATUSES.includes(request.status)) summary.quoting_count += 1;
      if (request.status === 'assigned') summary.assigned_count += 1;
      if (request.status === 'cancelled') summary.cancelled_count += 1;
      if (dashboardTag === 'pending_selection') {
        summary.pending_selection_count += 1;
        todo.pending_selection.push(item);
      }
      if (dashboardTag === 'expiring_soon') {
        summary.expiring_soon_count += 1;
        todo.expiring_soon.push(item);
      }
      if (dashboardTag === 'no_quote') {
        summary.no_quote_count += 1;
        todo.no_quote.push(item);
      }
      return item;
    }));

    const driverRegionSummary = await buildRegionSummary();

    return {
      success: true,
      summary,
      todo,
      driver_region_summary: driverRegionSummary,
      requests,
    };
  } catch (error) {
