...
// 客户自己的联系方式不进响应(仅作用于 customer 对象,不动酒店电话);web 直写/旧快照兜底
...
function toTransferRequest(request, quotes, assignedTransport = null, assignedTransportSource = 'none') {
  ...
  const routeText = request.route_text || [request.pickup || request.pickup_location || '', request.dropoff || request.dropoff_location || ''].filter(Boolean).join(' -> ');
  return {
    request_id: request._id,
    service_type: request.service_type || 'transfer',
    service_type_label: request.service_type_label || '',
    task_title: request.task_title || '',
    title: request.task_title || (request.request_no ? `用车方案 ${request.request_no}` : 'Farland 用车方案'),
...
function timelineToDestinationCards(day) {
  ...
  return items.map((item, index) => ({
    ...
    card_type: item.card_type || item.cardType || item.item_type || item.type || 'itinerary',
    sequence: index + 1,
    total_count: item.total_count || items.length,
...
function toTransportOrderSummary(transfer) {
  ...
  return {
    ...
    service_type: transfer.service_type || 'transfer',
    task_title: transfer.task_title || '',
    service_type_label: transfer.service_type_label || '',
    title: transfer.task_title || transfer.title || (transfer.service_type === 'charter' ? '今日包车服务' : '今日接送安排'),
    status_text: '已分配司机',
    order_status: 'assigned',
    pickup: transfer.pickup || '',
    dropoff: transfer.dropoff || '',
    pickup_time_text: transfer.pickup_time_text || '',
    route_text: transfer.route_text || [transfer.pickup || '', transfer.dropoff || ''].filter(Boolean).join(' -> '),
    time_summary: transfer.time_summary || '',
    flight_summary: transfer.flight_summary || '',
    task_description: transfer.task_description || '',
    estimated_drive_time: transfer.estimated_drive_time || '',
    estimated_distance: transfer.estimated_distance || '',
    flight_no: transfer.flight_no || '',
    terminal: transfer.terminal || '',
    scheduled_flight_time: transfer.scheduled_flight_time || '',
    driver_arrive_time: transfer.driver_arrive_time || '',
...
function buildCustomerVisitCard(booking, index) {
  ...
  const meetingPoint = isVisitObject(booking.meetingPoint) ? booking.meetingPoint
    : (isVisitObject(booking.meeting_point) ? booking.meeting_point : {});
  const arrival = isVisitObject(booking.arrival) ? booking.arrival : {};
  ...
  const weatherRaw = isVisitObject(booking.weather) ? booking.weather : null;
...
function deriveSnapshotDailyCards(snapshot, hotelCards) {
  ...
  return days.map((day, index) => {
    ...
    return sanitizeCustomerObject({
      ...
      city: day.city || '',
      weather_text: day.weather_text || day.weatherText || '',
      weather_kind: day.weather_kind || day.weatherKind || '',
      weather_source: day.weather_source || '',
      weather_mode: day.weather_mode || '',
      weather_generated_at: day.weather_generated_at || '',
...
function collectTripData(trips) {
  ...
  const visitCards = snapshots.reduce((acc, snapshot) => acc.concat(snapshot.visit_cards || []), []);
  return {
...
exports.main = async () => {
  ...
  return {
    ...
    visit_cards: tripData.visit_cards || [],
